using BlazorServer.FacadePatternExample.Data;
...
builder.Services.AddDbContext<FacadePatternContext>(options => options.UseSqlite(builder.Configuration.GetConnectionString("FacadePattern")));
...
async Task EnsureDbIsMigrated(IServiceProvider services)
{
    ...
    using var ctx = scope.ServiceProvider.GetService<FacadePatternContext>();
    if (ctx != null)
    {
